...
function checkAdminAuth() {
  const isAuth = sessionStorage.getItem('nethras_admin_authenticated') === 'true';
  if (isAuth) {
    if (adminAuthScreen) adminAuthScreen.style.display = 'none';
    if (adminMainWrapper) adminMainWrapper.style.display = 'flex';
    loadAdminData();
  } else {
    if (adminAuthScreen) adminAuthScreen.style.display = 'flex';
    if (adminMainWrapper) adminMainWrapper.style.display = 'none';
  }
}
...
if (adminLoginForm) {
  adminLoginForm.addEventListener('submit', (e) => {
    e.preventDefault();
    const userVal = document.getElementById('adminUserVal').value.trim();
    const passVal = document.getElementById('adminPassVal').value.trim();

    // Default Credentials: username: admin | password: admin (or nethras123)
    const storedPass = localStorage.getItem('nethras_admin_password') || 'admin';

    if ((userVal.toLowerCase() === 'admin' || userVal.toLowerCase() === 'manager') && (passVal === storedPass || passVal === 'nethras123' || passVal === 'admin')) {
      sessionStorage.setItem('nethras_admin_authenticated', 'true');
      showAdminToast(`<i class="fa-solid fa-lock-open"></i> Welcome back, Admin!`);
      checkAdminAuth();
    } else {
      showAdminToast(`<i class="fa-solid fa-circle-xmark" style="color:#d32f2f;"></i> Incorrect Admin Username or Password!`);
    }
  });
}
...
if (adminLogoutBtn) {
  adminLogoutBtn.addEventListener('click', () => {
    sessionStorage.removeItem('nethras_admin_authenticated');
    showAdminToast(`<i class="fa-solid fa-lock"></i> Logged out of Admin Panel.`);
    checkAdminAuth();
  });
}
...
if (adminPasswordForm) {
  adminPasswordForm.addEventListener('submit', (e) => {
    ...
    localStorage.setItem('nethras_admin_password', newPass);
    showAdminToast(`<i class="fa-solid fa-key"></i> Admin security password updated successfully!`);
